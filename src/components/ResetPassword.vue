  ...
  <div class="reset-password-overlay">
    <div :class="['reset-password-modal', `mode-${modeType}`]">
      ...
      <div class="header-area">
        ...
        <p v-if="modeType === '3'" class="modal-subtitle">{{ t('bindPhoneModal.subtitle') }}</p>
      </div>

      <div class="form-container">
        <div class="form-group" v-if="modeType === '2'">
          <label class="form-label">{{ t('resetPasswordModal.accountName') }}</label>
        ...
        <div class="form-group" v-if="['0', '1', '3'].includes(modeType)">
          <label class="form-label">{{ t('resetPasswordModal.phone') }}</label>
          <div class="input-wrapper" :class="{ 'disabled-wrapper': isFromSettings }">
            <span class="prefix">+86</span>
            <span class="divider"></span>
            <input type="tel" maxlength="11" v-model="formData.phone" class="form-input"
              :placeholder="t('LoginPopUpPage.enterPhoneNumber')" :disabled="isFromSettings"
              @input="handlePhoneInput" />
          </div>
        </div>

        <div class="form-group" v-if="['1', '2'].includes(modeType)">
          <label class="form-label">{{ t('resetPasswordModal.oldPassword') }}</label>
          <div class="input-wrapper">
            <input type="password" v-model="formData.oldPassword" class="form-input"
              :placeholder="t('resetPasswordModal.enterOldPassword')" />
          </div>
        </div>

        <div class="form-group" v-if="['0', '3'].includes(modeType)">
          <label class="form-label">{{ t('resetPasswordModal.code') }}</label>
          <div class="input-wrapper code-wrapper">
            <input maxlength="4" v-model="formData.code" type="tel" class="form-input"
        ...
        <template v-if="['0', '1', '2'].includes(modeType)">
          <div class="form-group">
            <label class="form-label">
              {{ t('resetPasswordModal.newPassword') }} <span class="label-hint">{{ t('resetPasswordModal.newPasswordHint') }}</span>
            </label>
            <div class="input-wrapper">
              <input maxlength="20" :type="showPwdOne ? 'text' : 'password'" v-model="formData.newPassword"
                class="form-input" :placeholder="t('resetPasswordModal.enterNewPassword')" />
          ...
          <div class="form-group">
            <label class="form-label">{{ t('resetPasswordModal.confirmPassword') }}</label>
            <div class="input-wrapper">
              <input maxlength="20" :type="showPwdTwo ? 'text' : 'password'" v-model="formData.confirmPassword"
                class="form-input" :placeholder="t('resetPasswordModal.reenterPassword')" />
...
const modalTitle = computed(() => {
  if (modeType.value === '0' && props.isSetPassword) {
    return t('resetPasswordModal.setPassword')
  }
  const titles = {
    '0': t('resetPasswordModal.resetPassword'),
    '1': t('resetPasswordModal.changePassword'),
    '2': t('resetPasswordModal.changePassword'),
    '3': t('resetPasswordModal.bindPhone'),
  }
  return titles[modeType.value] || t('resetPasswordModal.changePassword')
})
...
const submitBtnText = computed(() => {
  ...
  //   return '设置密码'
  // }
  return modeType.value === '3' ? t('bindPhoneModal.bind') : t('resetPasswordModal.submitReset')
})
...
const fetchSmsCode = async () => {
  if (!formData.phone || formData.phone.length !== 11) {
    return ElMessage.warning(t('LoginPopUpPage.enterPhoneNumber'))
  }
...
const handleSubmit = async () => {
  try {
    ...
    if (['0', '1', '2'].includes(modeType.value)) {
      if (!formData.newPassword) return ElMessage.error(t('resetPasswordModal.enterNewPassword'))
      ...
      if (!formData.confirmPassword) return ElMessage.error(t('resetPasswordModal.reenterPassword'))
      if (formData.newPassword !== formData.confirmPassword) {
        return ElMessage.error(t('resetPasswordModal.passwordMismatch'))
      }
    }
      ...
      case '0': // 找回/重置密码 (短信验证)
        {
          ...
          if (String((res as any).code) === '0000') {
            if (props.isSetPassword) {
              ElMessage.success(t('resetPasswordModal.passwordSetSuccess'))
              setTimeout(async () => {
              ...
              ElMessage.success(t('resetPasswordModal.passwordResetSuccess'))
              setTimeout(async () => {
                await userStore.logout() // 修改成功后退出登录
              }, 500); // 等待0.5秒防止卡顿
              emit('success', modeType.value)
            ...
            ElMessage.error(res.msg || t('resetPasswordModal.actionFailed'))
          }
      ...
      case '1': // 个人修改密码 (旧密码验证)
        {
          let res = null;
          if (!formData.phone) return ElMessage.error(t('LoginPopUpPage.enterPhoneNumber'))
          if (!formData.oldPassword) return ElMessage.error(t('resetPasswordModal.enterOldPassword'))
          ...
          if (String((res as any).code) === '0000') {
            ElMessage.success(t('resetPasswordModal.passwordChangeSuccess'))
      ...
      case '3': // 绑定手机号
        {
          ...
          if (String((res as any).code) === '0000' && res.success === true) {
            ...
            ElMessage.success(t('bindPhoneModal.bindSuccess'))
            emit('success', modeType.value)
            emit('close')
          } else {
            ElMessage.error(res.msg)
